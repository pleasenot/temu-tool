...
pricingRouter.get('/templates', (_req, res) => {
  const db = getDb();
  const templates = db.prepare('SELECT * FROM pricing_templates ORDER BY updated_at DESC').all();
...
pricingRouter.post('/templates', (req, res) => {
  ...
  db.prepare('INSERT INTO pricing_templates (id, name, default_values, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
    .run(id, name, JSON.stringify(defaultValues), now, now);
...
pricingRouter.put('/templates/:id', (req, res) => {
  ...
  db.prepare('UPDATE pricing_templates SET name = ?, default_values = ?, updated_at = ? WHERE id = ?')
    .run(name, JSON.stringify(defaultValues), now, req.params.id);
...
pricingRouter.delete('/templates/:id', (req, res) => {
  const db = getDb();
  db.prepare('DELETE FROM pricing_templates WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
pricingRouter.post('/apply', (req, res) => {
  ...
  const upsert = db.prepare(`
    INSERT INTO product_pricing (product_id, template_id, overrides)
    VALUES (?, ?, ?)
    ON CONFLICT(product_id, template_id) DO UPDATE SET overrides = excluded.overrides
  `);

  const applyAll = db.transaction(() => {
    for (const productId of productIds) {
      const productOverrides = overrides?.[productId] ? JSON.stringify(overrides[productId]) : null;
      upsert.run(productId, templateId, productOverrides);
    }
  });
  ...
  db.prepare(
    `UPDATE products SET status = 'priced' WHERE id IN (${productIds.map(() => '?').join(',')}) AND status = 'mockup_ready'`
  ).run(...productIds);
